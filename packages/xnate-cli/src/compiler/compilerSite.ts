...
import { INavPcItem, IConfigRouter } from '../interface/configType';
...
const getRootRouteDoc = () => {
  ...
  const routeMap = navs.filter((n: INavPcItem) => n.index).map((m: INavPcItem) => m.path);

  const configRoute: IConfigRouter[] = [];
